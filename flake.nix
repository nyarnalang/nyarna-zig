  ...
  outputs = { self, zicross, utils, filter, nixpkgs }:
    ...
    eachSystem allSystems (system:
      let
        ...
        version = if release then
          version_base
        else
          "${version_base}-${self.shortRev or self.lastModifiedDate}";
        handler_gen = {
          name = "handler_gen";
        ...
        buildNyarna = { pname, src, genTests ? false, ... }@args:
          pkgs.buildZig (args // {
            inherit version src;
            postConfigure = ''
        ...
        nyarna_cli = buildNyarna {
          ...
          src = filter.lib.filter {
            root = ./.;
            exclude =
              [ ./flake.nix ./flake.lock ./src/wasm.zig ./src/nyarna.js ];
          };
          genTests = true;
